...
function cfo_admin_v1_page() {
    ...
    <div class="cfo-admin">
      ...
      <div class="cfo-admin-table-wrap"><table class="cfo-admin-table"><thead><tr><th>Artiste</th><th>Init</th><th>Catalogue</th><th>ISRC</th><th>Inclus</th><th>Exclus</th><th>Review</th><th>Profil</th><th>Streaming</th><th>Social</th><th>Airplay</th><th>Charts</th><th>Events</th><th>RAW</th><th>Dernière collecte</th></tr></thead><tbody>
      <?php foreach ($artists as $a):
        ...
        <tr><td><strong><?php echo esc_html($a['name'] ?? '—'); ?></strong></td><td><span class="cfo-admin-status <?php echo esc_attr($cls); ?>"><i class="cfo-admin-dot"></i><?php echo esc_html($label); ?></span></td><td><?php echo intval($a['tracks'] ?? 0); ?></td><td><?php echo intval($a['with_isrc'] ?? 0); ?></td><td><?php echo intval($a['included'] ?? 0); ?></td><td><?php echo intval($a['excluded'] ?? 0); ?></td><td><?php echo intval($a['review'] ?? 0); ?></td><td><?php echo intval($a['profile_n'] ?? 0) ? '✓ '.intval($a['profile_n']) : '—'; ?></td><td><?php echo intval($a['streaming_n'] ?? 0) ? '✓ '.intval($a['streaming_n']) : '—'; ?></td><td><?php echo intval($a['social_n'] ?? 0) ? '✓ '.intval($a['social_n']) : '—'; ?></td><td><?php echo intval($a['airplay_n'] ?? 0) ? '✓ '.intval($a['airplay_n']) : '—'; ?></td><td><?php echo intval($a['charts_n'] ?? 0) ? '✓ '.intval($a['charts_n']) : '—'; ?></td><td><?php echo intval($a['events_n'] ?? 0) ? '✓ '.intval($a['events_n']) : '—'; ?></td><td><?php echo number_format_i18n(intval($a['raw_count'] ?? 0)); ?></td><td><?php echo !empty($a['last_raw_at']) ? esc_html(wp_date('d/m/Y H:i', strtotime($a['last_raw_at']))) : '—'; ?></td></tr>
